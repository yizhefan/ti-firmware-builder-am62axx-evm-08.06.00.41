...
/*!
    ...
    \tableofcontents
     ...
     \section TIOVX_USAGE_REC_STATICALLY Statically Allocated Structures
       ...
       - The utility function, \ref tivxPrintAllResourceStats, prints that maximum value of each parameter as well as the value being used currently
       - The utility function, \ref tivxQueryResourceStats, provides information about the parameter values of a specific resource given as a parameter to the function
       - The utility function, \ref tivxExportAllResourceMaxUsedValueToFile, generates a new config file called "tivx_config_generated.h" at VX_TEST_DATA_PATH.  This config file contains optimized parameter values.
       - All of the max values are documented in the following link: \ref group_vx_framework_config
       - All of these API's are documented further in the following link: \ref group_tivx_ext_host
...
/*!
    ...
    are implemented.  TIOVX is no exception.  The following list contains guidelines and assumptions that User Target Kernel callback
    implementers should follow for proper usage:
...
/*!
    ...
    File to change: source/platform/psdk_j7/common/tivx_platform_psdk_j7.h
    ...
    - A72 Targets: source/platform/psdk_j7/common/tivx_target_config_mpu1_0.c
    - R5F Targets: source/platform/psdk_j7/rtos/tivx_target_config_mcu2.c
    ...
    is tivxTargetCreate().  Each of these files are slightly different in the way that they call this API.  Regardless of the file, the tivxTargetCreate() must be called
...
/*!
    ...
    - As mentioned during the "Create" section, the amount of statically allocated structures in non-cached memory can be queried and modified. For more information on how to query
      and adjust these values, please see \ref TIOVX_USAGE_REC_STATICALLY.
...
/*!
    ...
    An example of this workaround can be found within the display node (tiovx/kernels_j7/hwa/display/vx_display_target.c).  Note: due to the limitation of
